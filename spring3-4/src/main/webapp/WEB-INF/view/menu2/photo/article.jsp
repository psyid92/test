...
//게시물 공감 개수
function countLikePhoto(num) {
	var url="<%=cp%>/photo/countLikePhoto";
...
function listAnswer(answer) {
	...
	var url="<%=cp%>/photo/listReplyAnswer";
...
function countAnswer(answer) {
	var url="<%=cp%>/photo/replyCountAnswer";
	$.post(url, {answer:answer}, function(data){
                 ...
                 <tbody>
                     ...
                     <tr>
                         <td colspan="2" style="height: 40px; padding-bottom: 15px; text-align: center;">
                              <button type="button" class="btn btn-default btn-sm wbtn" style="background: white;" onclick="sendLikePhoto('${dto.num}')"><span class="glyphicon glyphicon-hand-up"></span> <span id="countLikePhoto">${countLikePhoto}</span></button>
                         </td>
                     </tr>
...
<c:if test="${sessionScope.member.userId==dto.userId || sessionScope.member.userId=='admin'}">	                		    
                		...
                		<td align="right">
                		    <button type="button" class="btn btn-default btn-sm wbtn" onclick="javascript:location.href='<%=cp%>/photo/list?${query}';"> 목록으로 </button>
                		</td>
                	</tr>
                </tfoot>
            </table>

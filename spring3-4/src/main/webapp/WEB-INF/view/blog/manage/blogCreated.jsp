...
function themeList() {
  ...
  function sendBlog() {
        ...
        var mode="${mode}";
        if(mode=="created")
            f.action = "<%=cp%>/nblog/created";
        else if(mode=="update")
            f.action = "<%=cp%>/nblog/update";
    ...
    <div style="width:700px; padding-top:25px; clear: both; margin: 0px auto;">
    		<form name="blogForm" method="post" enctype="multipart/form-data">
			  ...
			  <tr height="40" >
			      ...
			      <td colspan="2"> 
                      <input type="file" name="upload" class="boxTF"  size="61"  style="width:98%; height: 20px;">			           
			       </td>
			  </tr> 
			  ...
			  <table style="width: 100%; margin: 0px auto; border-spacing: 0px;">
			     <tr height="45"> 
			      <td align="center" >
					   <button type="button" class="btn1" onclick="sendBlog();"> ${mode=="created"?"등록완료":"수정완료"} </button>
                       <button type="button" class="btn1" onclick="javascript:history.back();"> ${mode=="created"?"등록취소":"수정취소"} </button>
                       <c:if test="${mode=='update'}">
                           <input type="hidden" name="blogSeq" value="${dto.blogSeq}">
